...
class WorkManager {
    ...
    setupWorkNavigation() {
        // Work link clicks are handled by NavigationManager.
        // This method is kept for compatibility but no longer
        // adds a duplicate document-level click handler.
    }
...
void main() { vec2 uv = (gl_FragCoord.xy - offset) / resolution; vec2 p = uv * 2.0 - 1.0; p.y *= resolution.y / resolution.x; float t; if (leaveTime > 0.0) { dir = -1.0; t = clamp(leaveTime * speed, 0.0, 1.0); } else { t = clamp((enterTime - delay) * speed, 0.0, 1.0); } t = (fract(t * 0.99999) - 0.5) * dir + 0.5; for (float i = 0.0; i < LAYERS; i++) { float s = cos(i) * 9.0 + 12.0; gl_FragColor += draw(uv, p, t, abs(s)); } gl_FragColor /= LAYERS; gl_FragColor *= smoothstep(0.0, 0.02, t); }`;
    ...
    exitWorkMode(targetPage) {
        ...
        // Clean the hidden class from all standard screens
        ['homeScreen', 'aboutScreen', 'contactScreen'].forEach(id => {
            const s = document.getElementById(id);
            if (s) { s.classList.remove('hidden'); }
        });
        
        // Only force-navigate to home when the back button is pressed
        // (targetPage is undefined). When NavigationManager calls this
        // with a specific targetPage, it handles the screen transition.
        if (!targetPage) {
